				...
				synchronized (this.synchronizationObject) {
					...
					if (this.localInstance != null) { // Instance is available
						if (this.allocatedResource == null) { // Instance is not used by another job
							...
							if(this.allocatedResource.assignedVertices().hasNext()){
							if(this.allocatedResource.assignedVertices().next().getExecutionGraph().getJobID() == jobID){
								System.out.println("Instance is used by this job \n");
								assignmentSuccessful = true;
							}else{
								assignmentSuccessful = false;
								System.out.println("Instance is used by another job");
								break;
							}
							}
						}
					}else{
						LOG.error("Not instance available");
					}
				}
