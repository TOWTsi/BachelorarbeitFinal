...
public final class RuntimeTaskContext implements BufferProvider, AsynchronousEventListener, TaskContext {
	...
	RuntimeTaskContext(final RuntimeTask task, final CheckpointState initialCheckpointState,
		...
		int nooc = 0;
		ArrayList<ChannelID> channelIDs = new ArrayList<ChannelID>();
		for (int i = 0; i < environment.getNumberOfOutputGates(); ++i) {
			...
			if (outputGate.isBroadcast()) {
				...
				channelIDs.addAll(outputGate.getOutputChannelIDs());
		...
		if (initialCheckpointState == CheckpointState.NONE) {
			...
			this.ephemeralCheckpoint = new EphemeralCheckpoint(task, this.numberOfOutputChannels, channelIDs,
	...
	public EphemeralCheckpoint getEphemeralCheckpoint() {
	...
	void reportExhaustionOfMemoryBuffers() throws IOException, InterruptedException {
		...
		LOG.info("Checkpoint state of " + this.task.getRuntimeEnvironment().getTaskNameWithIndex() + " is UNDECIDED and requests decision");
		this.ephemeralCheckpoint.requestCheckpointDecision();
