...
public final class RuntimeTask implements Task, ExecutionObserver {
	...
	private FailureReport failureRep;
	 ...
	 */
	@Override
	public void executionStateChanged(ExecutionState newExecutionState, final String optionalMessage) {
		...
		if (newExecutionState == ExecutionState.FAILED) {
			if(this.failureRep == null){
				this.failureRep =  this.getRuntimeEnvironment().getFailureReport();
			}
			this.failureRep.setMessage(optionalMessage);
			LOG.error(optionalMessage);
		}
		
//		if (newExecutionState == ExecutionState.CANCELING && this.executionState == ExecutionState.FINISHING){
...
//		}
	...
	public void markAsFailed() {
		failureRep = this.getRuntimeEnvironment().getFailureReport();
	...
	public FailureReport getFailureReport() {
		if(failureRep == null){
			return this.getRuntimeEnvironment().getFailureReport();
		}
		return failureRep;
	}
	
	public void setEnvironment(RuntimeEnvironment re){
		this.environment = re;
	}
}
